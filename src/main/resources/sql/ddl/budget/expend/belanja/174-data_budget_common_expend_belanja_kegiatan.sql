create or replace table data_budget_common_expend_belanja_kegiatan (
	...
	constraint ck_data_budget_common_expend_belanja_kegiatan_01 check (id = concat_ws('|', unit_id, bidang_id, kegiatan_id)),
	constraint ck_data_budget_common_expend_belanja_kegiatan_02 check (sebelum = belanja_operasi_sebelum + belanja_modal_sebelum + belanja_tidak_terduga_sebelum + belanja_transfer_sebelum),
	constraint ck_data_budget_common_expend_belanja_kegiatan_03 check (total = belanja_operasi + belanja_modal + belanja_tidak_terduga + belanja_transfer),
	constraint ck_data_budget_common_expend_belanja_kegiatan_04 check (program_code = if(program_id not like '0.00.01%', program_id, concat(bidang_id, substring(program_id, 5)))),
	constraint ck_data_budget_common_expend_belanja_kegiatan_05 check (kegiatan_code = if(kegiatan_id not like '0.00.01%', kegiatan_id, concat(bidang_id, substring(kegiatan_id, 5)))),
	constraint fk_data_budget_common_expend_belanja_kegiatan_01 foreign key (skpd_id) references data_master_organisasi (id),
	constraint fk_data_budget_common_expend_belanja_kegiatan_02 foreign key (skpd_id, unit_id) references data_master_organisasi (skpd_id, unit_id),
	constraint fk_data_budget_common_expend_belanja_kegiatan_03 foreign key (urusan_id) references data_master_urusan (id),
