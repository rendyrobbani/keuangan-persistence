insert into keuangan_dev.data_master_priority_of_nasional (id, from_year, into_year, year, number, name, sipd_name, is_locked, locked_at, locked_by, created_at, created_by, updated_at, updated_by, is_deleted, deleted_at, deleted_by)
select *
from (
	select id                                                     as id
	...
	from espresso_2025_preproduction.data_master_label_pusat
) t
on duplicate key update from_year  = t.from_year
                      , into_year  = t.into_year
                      , year       = t.year
                      , number     = t.number
                      , name       = t.name
